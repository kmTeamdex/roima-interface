...
const getReffer = (address?: string | null) => {
  var port = window.location.port ? `:${window.location.port}` : '';
  var refUrl = `${window.location.protocol}//${window.location.hostname}${port}/#/app/swap`;
  if (address) {
    refUrl = `${refUrl}?r=${address}`;
    return refUrl;
  }
  return refUrl;
};
    ...
    return (
      <>
        ...
        <Flex sx={{ flexDirection: 'column', marginTop: 16 }}>
          <Text
            sx={{ textAlign: 'center', fontSize: '14', color: 'white.300', fontWeight: '400', fontStyle: 'inherit' }}
          >
          ...
          {account ? (
            ...
            <Text
              sx={{ textAlign: 'center', fontSize: '14', color: 'mint.300', fontWeight: 'bold', fontStyle: 'inherit' }}
            >
